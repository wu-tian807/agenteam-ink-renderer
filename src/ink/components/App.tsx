...
import { logForDebugging, pasteDebug, pasteDebugEnabled } from '../../utils/debug.js';
...
export default class App extends PureComponent<Props, State> {
  ...
  readonly PASTE_TIMEOUT = 500; // Longer timeout for an incomplete CSI mid-paste
  // IN_PASTE watchdog: Cursor's bundled node-pty (1.1.0-beta42) drops the
  // PASTE_END marker on every bracketed-paste under raw-mode TTY (see
  // anthropics/claude-code#34529 and Cursor forum #154801 — backpressure
  // bug in the host PTY layer). Without this watchdog the parser sticks in
  // IN_PASTE forever and silently eats every subsequent keystroke; the UI
  // looks frozen even though the event loop is fully alive. We force-flush
  // pasteBuffer as a single paste key once stdin has been idle this long.
  //
  // Tuning: empirical max inter-chunk gap during a healthy paste over our
  // SSH link is ~1.7s. 2.5s gives ~50% safety margin while keeping the
  // user-perceived wait under 3s. Bumping higher only helps the rare slow-
  // SSH case at the cost of every paste feeling sluggish; bumping lower
  // risks splitting a legitimate paste in half (which would then leak the
  // tail bytes as raw keystrokes / `?` glyphs).
  readonly PASTE_WATCHDOG = 2500;
      ...
      rows: this.props.terminalRows
        ...
        exit: this.handleExit
          ...
          internal_querier: this.querier
            ...
            {/* @ts-ignore React Compiler compiled providers */}
  ...
  override componentDidMount() {
    ...
    // CRITICAL: Force stdout to non-blocking mode. On POSIX TTYs, Node.js
    // writes are synchronous by default — when the SSH channel buffer fills
    // up (e.g. during a large paste, Cursor client busy parsing input),
    // stdout.write() BLOCKS the entire event loop. This freezes the renderer
    // even though the terminal client itself stays responsive (link clicks,
    // selection still work because they're client-side).
    //
    // Setting non-blocking makes writes return immediately via libuv's
    // internal queue; the kernel handles draining asynchronously. Pasting
    // large content no longer deadlocks the renderer.
    const stdoutHandle = (this.props.stdout as unknown as { _handle?: { setBlocking?: (b: boolean) => void } })._handle;
    if (stdoutHandle?.setBlocking) {
      try { stdoutHandle.setBlocking(false); } catch { /* best-effort */ }
    }
    const stderrHandle = (this.props.stderr as unknown as { _handle?: { setBlocking?: (b: boolean) => void } })._handle;
    if (stderrHandle?.setBlocking) {
      try { stderrHandle.setBlocking(false); } catch { /* best-effort */ }
    }
    pasteDebug('mount');
  }
  ...
  // Helper to flush incomplete escape sequences and stuck IN_PASTE buffers.
  flushIncomplete = (): void => {
    this.incompleteEscapeTimer = null;
    const stuckInPaste = this.keyParseState.mode === 'IN_PASTE';
    if (!this.keyParseState.incomplete && !stuckInPaste) return;
    ...
    // lone ESC, or the final paste chunks). Node's event loop runs the
    // timers phase before the poll phase, so when a heavy render blocks
    // the loop past the timeout, this timer fires before the queued
    // readable event even though the bytes are already buffered. Re-arm
    // with a SHORT delay so handleReadable drains stdin next and the
    // re-armed paste-watchdog (in processInput) takes over the long wait.
    ...
    if (stuckInPaste) {
      pasteDebug('paste-watchdog.fire', { incomplete: this.keyParseState.incomplete.length });
    }
  ...
  processInput = (input: string | Buffer | null): void => {
    const wasInPaste = this.keyParseState.mode === 'IN_PASTE';

    // Parse input using our state machine
    const [keys, newState] = parseMultipleKeypresses(this.keyParseState, input);
    this.keyParseState = newState;

    const isInPaste = this.keyParseState.mode === 'IN_PASTE';

    // Diagnostic log only fires for paste-related transitions; gated by
    // INK_PASTE_DEBUG=1 so the constant-cost path (every keystroke) stays cheap.
    if (pasteDebugEnabled() && (wasInPaste || isInPaste)) {
      pasteDebug('processInput', {
        inputLen: input == null ? 0 : input.length,
        keys: keys.length,
        wasInPaste,
        isInPaste,
        incomplete: this.keyParseState.incomplete.length,
      });
    }
    ...
    // Arm a flush timer when either:
    //   1. tokenizer holds a partial escape sequence (NORMAL_TIMEOUT or
    //      PASTE_TIMEOUT depending on mode), OR
    //   2. we are mid-bracketed-paste and stdin might dry up before PASTE_END
    //      arrives (PASTE_WATCHDOG, see field comment).
    // Re-arm on every processInput so a healthy paste (constant chunks) keeps
    // pushing the deadline out — only a true stall trips the watchdog.
    const armWatchdog = this.keyParseState.incomplete || this.keyParseState.mode === 'IN_PASTE';
    if (armWatchdog) {
      if (this.incompleteEscapeTimer) {
        clearTimeout(this.incompleteEscapeTimer);
      }
      const timeout = this.keyParseState.mode === 'IN_PASTE'
        ? (this.keyParseState.incomplete ? this.PASTE_TIMEOUT : this.PASTE_WATCHDOG)
        : this.NORMAL_TIMEOUT;
      this.incompleteEscapeTimer = setTimeout(this.flushIncomplete, timeout);
    }
  };
  handleReadable = (): void => {
    ...
    try {
      ...
      if (combined) {
        if (chunks > 1 || combined.length > 200) {
          pasteDebug('handleReadable', { chunks, totalLen: combined.length });
        }
        this.processInput(combined);
      }
    } catch (error) {
...
export function handleMouseEvent(app: App, m: ParsedMouse): void {
  ...
  if (m.action === 'press') {
    if ((m.button & 0x20) !== 0 && baseButton === 3) {
      // Mode-1003 motion with no button held.
