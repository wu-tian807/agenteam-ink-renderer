...
import { color } from '../ink/components/design-system/color.js'
import { BLOCKQUOTE_BAR } from '../lib/figures.js'
...
export function formatToken(
  ...
  switch (token.type) {
    ...
    case 'code': {
      const codeText = highlight
        ? highlight.highlight(token.text, {
            language: token.lang && highlight.supportsLanguage(token.lang) ? token.lang : 'plaintext',
          })
        : token.text
      const BG_ON = '\x1b[48;2;40;42;46m'
      const BG_OFF = '\x1b[49m'
      return (codeText as string).split(EOL).map((line: string) => `${BG_ON} ${line} ${BG_OFF}`).join(EOL) + EOL
    }
    case 'codespan': {
