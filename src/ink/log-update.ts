...
function writeCellWithStyleStr(
  ...
  // Force terminal cursor to the correct column after any wide char.
  // needsCompensation cases (VS16, Unicode 12+ Symbols) already need this for
  // terminals with stale wcwidth tables. But any Wide cell can cause cursor
  // desync on terminals (e.g. Cursor/xterm.js on Windows) whose wcwidth tables
  // disagree with stringWidth — the CHA is a no-op on correct terminals and a
  // fix on terminals that render a 2-wide glyph as 1-wide.
  if (cellWidth === 2) {
