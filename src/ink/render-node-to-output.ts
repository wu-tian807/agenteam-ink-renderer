...
function wrapWithOsc8Link(text: string, url: string): string {
  // xterm.js re-evaluates OSC 8 link hover decorations on every buffer
  // write, causing flicker during periodic renders (spinner ~80ms).
  // Skip OSC 8; xterm.js detects URLs natively for Ctrl+Click.
  if (isXtermJs()) return text
    ...
    if (node.nodeName === 'ink-raw-ansi') {
      ...
      if (isScrollY) {
        ...
        if (content && contentYoga) {
          ...
          if (hint && prevScreen && safeForFastPath) {
            cleanupCulledCaches(content, scrollTop, scrollTop + innerHeight)
          }
...
/**
 * Post-render cleanup for DECSTBM fast path: walk direct children and drop
 * stale nodeCache entries for any child outside the full ScrollBox viewport.
 * Called AFTER both edge and stable passes complete, so blit/shift is done
 * and the stale entries serve no further purpose. Eliminates ghost hits in
 * hitTest/dispatchHover caused by preserveCulledCache retaining old coords.
 */
function cleanupCulledCaches(
  node: DOMElement,
  scrollTopY: number,
  scrollBottomY: number,
): void {
  for (const childNode of node.childNodes) {
    ...
    const cached = nodeCache.get(childElem)
    if (!cached) continue
    const top = cached.top ?? childElem.yogaNode?.getComputedTop()
    if (top === undefined) continue
    if (top + cached.height <= scrollTopY || top >= scrollBottomY) {
