...
>({
  ...
  createInstance(
    ...
    if (hostContext.isInsideText && originalType === 'ink-box') {
      const propsStr = Object.keys(newProps).filter(k => k !== 'children').join(', ')
      const ownerChain = internalHandle ? getOwnerChain(internalHandle).join(' > ') : 'unknown'
      throw new Error(`<Box> can't be nested inside <Text> component (owners: ${ownerChain}) (Box props: ${propsStr})`)
    }
