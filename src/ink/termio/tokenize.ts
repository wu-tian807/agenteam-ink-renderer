    ...
    switch (result.state) {
      ...
      case 'apc':
        if (code === C0.BEL) {
          ...
          // Unterminated OSC/DCS/APC followed by a CSI start. Common when
          // pasting content with hyperlinks (`\x1b]8;;url` without BEL/ST)
          // immediately followed by the bracketed-paste end marker
          // `\x1b[201~`. Without this branch, the OSC state happily
          // consumes ESC+[+201+~ as OSC body and PASTE_END never reaches
          // the parser — the session sticks in IN_PASTE forever.
          // See claude-code#41598 for the full state-machine trace.
          // Emit the unterminated sequence as-is and re-enter escape
          // state so the CSI is parsed normally.
          emitSequence(data.slice(seqStart, i))
          seqStart = i
          result.state = 'escape'
          i++
